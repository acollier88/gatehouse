...
- `gated_fetch` does not follow redirects: policy cleared one host, and a
  redirect would reach a host no rule ever saw. A 3xx comes back as a tool
  error naming the `Location` so the caller can re-submit it for a fresh check.
- `gated_fetch` accepts `http`/`https` only; other schemes have no host for
  policy to match on.
